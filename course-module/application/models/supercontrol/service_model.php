...
/*function show_service_main(){
		$sql ="select * from np_servicecategory_details WHERE pid=0";
		//$this->db->where('pid', 0);
		$query = $this->db->query($sql);
		return($query->num_rows() > 0) ? $query->result(): NULL;
	}
function show_service_sub($id)
	{
		$sql ="select * from np_servicecategory_details";
...
function service_edit($id, $datalist){
...
function delete_service($id){
		//=================******************=================
		$query = $this->db->get_where('sm_category',array('parent_id' => $id));
    	if ($query->num_rows() < 1) {
    		$this -> db -> where('category_id', $id);

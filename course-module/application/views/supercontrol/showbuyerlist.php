    ...
    <div class="page-content">
      ...
      <div class="page-bar">
        <ul class="page-breadcrumb">
          ...
          <li> <span>Show Buyer List </span> </li>
        </ul>
      </div>
      <!-- END PAGE BAR -->
      <!-- BEGIN PAGE TITLE-->
              ...
              <div class="tab-pane active" id="tab_0">
                <div class="portlet box blue-hoki">
                  <div class="portlet-title">
                    <div class="caption"> <i class="fa fa-gift"></i>Show Buyer</div>
                    ...
                    <div id="mydiv">
                      ...
                      ?>
                        <tr class="table table-striped table-bordered table-hover table-checkable order-column dt-responsive" id="sample_1">
                          ...
                          <td><?php if($i->buyer_image==""){?>
                            No Image
                            <?php }else{?>
                            <img src="<?php echo base_url()?>/buyer/<?php echo $i->buyer_image;?>" width="150" height="100" style="border: 2px solid #ddd;"/>
                            <?php }?></td>
                          <td  style="max-width:200px;"><?php echo $i->buyer_title;?></td>
                          ...
                          <td  style="max-width:250px;"><div class="form-group">
                              <div class="col-md-5">
                                <select name="blog_status" id="stachange" onchange="f1(this.value,<?php echo $i->id ;?>)" style="padding:4px;">
                                  <option value="1" <?php if($i->buyer_status==1){?> selected="selected"<?php }?>>Active</option>
                                  <option value="0" <?php if($i->buyer_status==0){?> selected="selected"<?php }?>>Inactive</option>
                                </select>
                                <input type="hidden" name="id" value="<?php echo $i->id;?>">
                              </div>
                            </div></td>
                            <td><a class="btn sbold yellow" href="<?php echo base_url()?>supercontrol/buyer/buyer_view/<?php echo $i->id?>"><i class="fa fa-eye"></i> View </a></td>
                          <td style="max-width:50px;"><a class="btn green btn-sm btn-outline sbold uppercase" href="<?php echo base_url()?>supercontrol/buyer/show_buyer_id/<?php echo $i->id; ?>">Edit</a></td>
                          <td style="max-width:50px;"><a class="btn red btn-sm btn-outline sbold uppercase" onclick="return confirm('Are you sure about this delete?');" href="<?php echo base_url()?>supercontrol/buyer/delete_buyer/<?php echo $i->id; ?>">Delete</a></td>
                        </tr>
...
//                    return  false;
					 //alert ("<?php echo base_url() ?>supercontrol/controllers/buyer/delete_multiple");
                    $.ajax({
						  
                        url: '<?php echo base_url() ?>supercontrol/buyer/delete_multiple',
                        type: 'post',
                        data: 'ids=' + checkValues
                    }).done(function(data) {
                        $("#respose").html(data);
						//location.reload();
						var newurl= '<?php echo base_url() ?>supercontrol/buyer/show_buyer';
...
<script>
		...
		$.ajax({
		type:"get",
		url:"<?php echo base_url(); ?>supercontrol/buyer/statusbuyer",
